...
public class Tomcat60ContainerAdaptor extends AbstractTomcatContainer {
  ...
  public boolean canBoundTo(String binding) {
    ...
    if (binding != null) {
      canBind |= binding.startsWith("Apache Tomcat/6.0");
      canBind |= binding.startsWith("JBossWeb/2.0");
      canBind |= binding.startsWith("JBoss Web/2.1");
      canBind |= binding.startsWith("NonStop(tm) Servlets For JavaServer Pages(tm) v6.");
      canBind |= (binding.startsWith("SpringSource tc") && binding.contains("/6.0"));
      canBind |= (binding.startsWith("VMware vFabric tc") && binding.contains("/6.0"));
    }
    return canBind;
  }
